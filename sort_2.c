  ...
  int min=-1;
    for (int i = 0; i<l-1; i++)
    {
      min=i;
        for (int j = i+1; j<l; j++)
        {
            if (strcmp(p[j].adder,p[min].adder) < 0)
            {      
                min=j;
            }
        }
        strcpy(temp, p[min].adder);
        strcpy(p[min].adder,p[i].adder);
...
char temp[50];
    ...
    for (int i = 0; i<l-1; i++)
    {
      min=i;
        for (int j = i+1; j<l; j++)
        {
            if (strcmp(blank[j].adder,blank[min].adder) < 0)
            {      
                min=j;
            }
        }
...
main(int argc, char *argv[])
{
  ...
  else if(argc==2)
  {
    if((fd = open(argv[1], 0)) < 0){
      printf(1,"Selction sort: cannot open %s\n", argv[1]);
    }
    sort(fd);
   ...
   else if(argc==4 && strcmp("-o",argv[1])==0)
  {
    ...
    printf(1, "Selction sort: cannot open destination %s\n", argv[2]);
    exit();
  } 
    sort_o(fd,dfd);
    close(fd);
