...
char temp[50];
    ...
    for (int i = 0; i<l-1; i++)
    {
        ...
        {
            if (strcmp(p[j].adder, p[j+1].adder) > 0)
            {      
                strcpy(temp, p[j].adder);
               strcpy(p[j].adder,p[j+1].adder);
...
char temp[50];
    ...
    for (int i = 0; i<l; i++)
    {
        for (int j = i + 1; j<l; j++)
        {
            if (strcmpo(blank[i].adder, blank[j].adder) > 0)
            {      
                strcpy(temp, p[j].adder);
               strcpy(p[j].adder,p[i].adder);
               strcpy(p[i].adder,temp);
...
main(int argc, char *argv[])
{
  ...
  else if(argc==2)
  {
    if((fd = open(argv[1], 0)) < 0){
      printf(1,"Bubbble sort: cannot open %s\n", argv[1]);
    }
    sort(fd);
   ...
   else if(argc==4 && strcmp("-o",argv[1])==0)
  {
    ...
    printf(1, "Bubbble sort: cannot open destination %s\n", argv[2]);
    exit();
  }
    sort_o(fd,dfd);
    close(fd);
